...
bool alarmEmpfangen = false;			          // Statusvariablen für Empfang 			  
bool alarmAn = false;								      
...
wandelt um & aktiviert funktionellen Alarm*/
...
/* ermöglicht manuelles de- & aktivieren 
...
void updateAlarm() {
	if(alarmAn){											    										// wenn Alarm aktiviert 
		...
		if (millis() - toggleZuletzt >= toggleInterval) {				 
			...
			digitalWrite(LEDR_PIN, ledStatus ? HIGH : LOW);				// LED & Buzzer togglen
...
void loop() {
	...
	if (WiFi.status() != WL_CONNECTED) {						// WLAN prüfen
