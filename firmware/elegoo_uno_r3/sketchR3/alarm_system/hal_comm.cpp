...
static bool zeileVerwerfen = false;


// Das Protokoll ist bewusst reines druckbares ASCII. NUL, DEL, Tabs und
// weitere Steuerzeichen machen immer die gesamte Zeile ungueltig.
static bool istErlaubtesZeichen(char c) {
  uint8_t wert = static_cast<uint8_t>(c);
  return wert >= 0x20 && wert <= 0x7E;
}
...
void hal_comm_sendeEvent(KommEvent evt) {
  switch (evt) {
    ...
    case EVT_PERSISTENZ_FEHLER:   Serial.println(F("STATUS:PERSISTENZ_FEHLER")); break;
  }
}
...
KommKommando hal_comm_leseKommando() {
  while (Serial.available() > 0) {
    ...
    if (c == '\n' || c == '\r') {
      // Nach einem Ueberlauf oder unzulaessigen Zeichen die komplette Zeile
      // verwerfen. So kann kein Suffix als gueltiges Kommando durchrutschen.
      if (zeileVerwerfen) {
        pufferIdx = 0;
        zeileVerwerfen = false;
        continue;
    ...
    // Zeichen sammeln
    else if (zeileVerwerfen) {
      continue;
    }
    else if (!istErlaubtesZeichen(c)) {
      pufferIdx = 0;
      zeileVerwerfen = true;
    }
    ...
    // Ueberlauf -> bis zum naechsten Zeilenende verwerfen
    else {
      pufferIdx = 0;
      zeileVerwerfen = true;
    }
